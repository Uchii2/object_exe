...
public class Game extends JPanel implements ActionListener{
	...
	private ArrayList<Alien3> aliens3; //list of visible aliens3, 追加：鎗分汰地
	...
	public void initBoss(){
		...
		aliens3.removeAll(aliens3);// 追加：鎗分汰地
	 ...
	 * 追加：鎗分汰地
	 */
	public void updateAliens3(){
				
				for(int i = 0; i < aliens3.size(); i++){
					
					Alien3 a = aliens3.get(i);
					if(a.isVisible())
						a.move();
					else{
						aliens3.remove(i);
				...
				if(spawn > 980 && aliens3.size() < 10){
					int posY = rand.nextInt(B_HEIGHT);
					int posX = rand.nextInt(B_WIDTH) + 400;
								
					if(posY < 76 && ennemies[0][2] == 0) aliens3.add(new Alien3(posX, 20)); //the condition on the enemies array is used to avoid an alien spawning on a line if there is already a wall in it
					else if(posY > 76 && posY < 132 && ennemies[1][2] == 0) aliens3.add(new Alien3(posX, 81));
					else if(posY > 132 && posY < 188 && ennemies[2][2] == 0) aliens3.add(new Alien3(posX, 142));
					else if(posY > 188 && posY < 244 && ennemies[3][2] == 0) aliens3.add(new Alien3(posX, 203));
					else if(posY > 244 && posY < 305 && ennemies[4][2] == 0) aliens3.add(new Alien3(posX, 264));		
				}
	...
	public void checkCollisions(){
        ...
        for(Alien2 alien2 : aliens2){
        	Rectangle rA2 = alien2.getBounds();
      ...
      //collision between the craft and the aliens
        ...
        for (Alien3 alien3 : aliens3){
            Rectangle rA3 = alien3.getBounds();

            if (rC.intersects(rA3)) { //the intersects method simply verify if the two bounds touch
            	alien3.setVisible(false); //if its the case, the alien disappear 
            	if(craft.isImmune()){
            		craft.setImmune(false); //if the craft was immune, the player doesn't lose a life
            		updateScorepan();}
            	else{
                ...
                alien3.playSound(); //sound the sprite plays on its death
            }
        }
        ...
        for (Missile m : ms) {
            ...
            for(Alien2 alien2 : aliens2){
            	...
            	if(rM.intersects(rA2)){
            		...
            		alien2.setLife(alien2.getLife()-1);
            		if(alien2.getLife() == 0){ //this alien needs to be shot twice 
            			alien2.setVisible(false);
            			score+=2;
            			updateScorepan();
            			alien2.playSound();
